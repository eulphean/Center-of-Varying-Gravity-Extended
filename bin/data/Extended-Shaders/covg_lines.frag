...
{    
    ...
    colB.r = colB.r + sin(u_time) * d * 0.2; 
    colB.g = colB.g + cos(u_time) * d * 0.3; 
    colB.b = colB.b + sin(u_time) * d * 0.4; 
    ...
    //st = st * (2.0 + abs(sin(u_time * 0.1)) * 100.0);
    float y = sin(u_time * 0.1); 
    float z = cos(u_time * 0.1);
    vec3 c = vec3(w.x, y, z);
    // col = mix (col, c, f1);
