...
{    
    ...
    float A = pattern(abs(p) * 10.0);
    vec2 B = tan(p-cPos) / 10.0  *  tan(p-cPos) / 10.0;
...
//float f = clamp((fbm4(abs(patternSeed*(p-cPos)/40.0)+ pattern(abs((p-cPos)*8.0)) - u_time*(upperLid+lowerLid)*.06)), 0.0, 1.0);
//float f = clamp((fbm4(abs(patternSeed*(p-cPos)/40.0)+ pattern(abs((p-cPos)*8.0)), 0.0, 1.0);